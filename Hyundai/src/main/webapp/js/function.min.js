...
var fn = (function() {
    ...
    return {
		...
		headerInit : function(){
			...
			// esuper searcharea start
			if ($(".esuper .search input").val() != "") {
				$(".esuper .searcharea").addClass("active");
			}

			$(document).mouseup(function(e){
				if ($(".esuper .searcharea").hasClass("active")) {
					if (!$(".esuper .search input, #header .search button").is(e.target) && !$(".popsearch").find("*").is(e.target)){
						if ($(".esuper .search input").val() == "") {
							$(".esuper .searcharea").removeClass("active");
						}
						$('.popsearch #esuDefaultsearch').fadeOut();
						$('.popsearch .autosearch').fadeOut();
					}
				}
			});
			// esuper searcharea end
...
$(document).ready(function() {
	...
	// esuper tab-menu
	var esuperTab = $('.esuper-tab');
	var esuperTabLength = esuperTab.length;
	
	if (esuperTabLength > 0) {
		
		esuperTab.find('.depth1 > li').on('click', function(){
			//$(this).addClass('active').siblings().removeClass('active');
			$(this).addClass('active').siblings().removeClass('active');

			var cHeight = $(this).find('.depth2').outerHeight(true);
			$(this).find('.clnb').css({'min-height': cHeight+'px'});
		});

		var depthList = $('.depth1 li.active .depth2 li').length;
		console.log(depthList);
		if(depthList > 6) {
			$('.clnb').css({'min-height':'120px'});
		}

		esuperTab.find('.depth2 > li').on('click', function() {
			$(this).addClass('active').siblings().removeClass('active');
		});
	...
	function scheduleTab() {
		var tabList  = $('.popesuperschedule .tabs li');
